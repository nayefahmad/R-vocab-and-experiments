...
# Choose the top users
# top_users <- sort(table(df2.cleaned.data$denom_PatientID), decreasing  = TRUE)
# head(top_users)
...
mod.cv <- xgb.cv(data = train.dmatrix, 
                 ...
                 nfold = 5, 
                 metrics = "auc", 
                 eta = 0.1,    # the learning rate (between 0.01 - 0.3)  
                 max_depth = 3,   
...
test.predicted.class <- as.numeric(test.predicted.prob > 0.5)
...
# 2018-12-24 Add-on: plot the AUC for the training and testing samples
mod.cv$dt %>% ## not work???
  select(-contains("std")) %>%
